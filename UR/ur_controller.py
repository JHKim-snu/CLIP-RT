...
class Arm:
    ...
    def to_home_pose(self):
        self.robot.movej(self.home_pose_vertical, acc=0.5, vel=0.5, relative=False)
        time.sleep(3)
        return
    ...
    def move_gripper(self, action, arm_len=0.15):
        ...
        self.robot.set_pose(trans, vel=0.5)
        time.sleep(2)
        ...
        if len(action) == 8:
            ...
            self.robot.movej(joint_pose, acc=0.5, vel=0.5, relative=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--getpose", dest="getpose", action="store_true")
    ...
    if args.getpose:
        ...
        action = [-0.2, 0.20, -0.27, 0, 0, 0, 0, 0]  # [x,y,z,rx,ry,rz,joint[5]]
